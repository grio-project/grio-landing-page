...
export default function StepTwo(props: Props) {
  ...
  const handleSubmit = () => {
    ...
    setDoc(doc(firestore, props.isMentor ? "mentor-formulario-cadastro" : "formulario-cadastro", email), {
      email: email,
      name: nome,
      gender: genero,
      job_title: profissao,
  ...
  updateTags();
  const handleTagList = (tag: string) => tags.has(tag) ? tags.delete(tag) : tags.add(tag);
  ...
  return (
    <>
      <StepTwoContainer>
        ...
        <StepTags>
            {
              checkboxes.map(checkItem => <CheckBoxButton
